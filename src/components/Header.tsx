...
const Header = () => {
  ...
  const getPageTitle = () => {
    if (location.pathname.includes("/upload")) return "Carica Documenti";
    if (location.pathname.includes("/interview")) return "Intervista AI";
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 h-20 lg:h-24 border-b border-transparent
          ${isScrolled
      ...
      >
        <div className="container mx-auto px-6 h-full flex items-center justify-between">
            ...
            {navItems.map(item => {
              ...
              return (
                <a
                  key={item.href}
                  href={item.href}
                  className="flex items-center gap-2 text-gray-700 font-semibold hover:text-accent"
                >
          ...
          {/* Mobile Menu Button */}
          <button
            className="lg:hidden p-3 rounded-xl border bg-white/90 backdrop-blur-sm"
              ...
              {navItems.map(item => {
                ...
                return (
                  <a
                    key={item.href}
                    href={item.href}
                    className="flex items-center gap-4 text-gray-700 font-semibold py-2"
