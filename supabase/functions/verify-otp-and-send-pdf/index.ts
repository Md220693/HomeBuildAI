...
async function sendEmailWithPostmark(to: string, subject: string, htmlBody: string, textBody: string, pdfUrl?: string) {
  ...
  try {
    const emailData: any = {
      From: 'HomeBuildAI <noreply@homebuildai.site>',
      To: to,
      Subject: subject,
      HtmlBody: htmlBody,
      TextBody: textBody,
...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Update lead status to show PDF is being generated
    ...
    const immediateResponse = new Response(
      JSON.stringify({
        success: true,
        message: 'OTP verificato! Generazione PDF in corso...',
        status: 'generating_pdf',
        lead_id: leadId
      }),
      {
    ...
    setTimeout(async () => {
      try {
        console.log('Starting background PDF generation for lead:', leadId);
        ...
        if (pdfError || pdfData?.error) {
          ...
          await supabase
            ...
            .update({ 
              ...
              last_error: pdfError?.message || pdfData?.error 
            })
            .eq('id', leadId);
        } else {
        ...
        if (pdfSuccess && pdfUrl) {
          ...
          console.log('Email sent status:', emailResult.success ? 'Success' : 'Failed');
        }
      } catch (bgError) {
